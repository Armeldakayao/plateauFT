...
import { Handshake, Send, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function CreatePartenaritPage() {
  ...
  const watchOrganizationType = watchedValues.organizationType
  const watchActivitySector = watchedValues.activitySector
  const watchPartnershipNature = watchedValues.partnershipNature
    ...
    switch (currentStep) {
      ...
      case 2:
        return !!(
          ...
          // @ts-ignore
          (values.organizationType !== "autre" || values.otherOrganizationType?.trim().length >= 1) &&
          values.activitySector?.trim().length >= 1 &&
          // @ts-ignore
          (values.activitySector !== "autre" || values.otherActivitySector?.trim().length >= 1) &&
          values.originCountry?.trim().length >= 2 &&
          values.originCity?.trim().length >= 2 &&
          values.creationYear >= 1900 &&
          values.creationYear <= new Date().getFullYear()
        )
      case 3:
        return !!(
          values.contactName?.trim().length >= 2 &&
          values.contactFunction?.trim().length >= 2 &&
          values.contactPhone?.trim().length >= 8 &&
          values.contactEmail?.includes("@") &&
          values.partnershipNature?.trim().length >= 1 &&
          // @ts-ignore
          (values.partnershipNature !== "autre" || values.otherPartnershipNature?.trim().length >= 1) &&
          values.concernedService?.trim().length >= 2
        )
      case 4:
        return !!(
          values.proposalDescription?.trim().length >= 10 &&
          values.mairieObjectives?.trim().length >= 10 &&
          values.structureObjectives?.trim().length >= 10 &&
          values.partnershipDuration?.trim().length >= 1 &&
          values.startDate?.trim().length >= 1
        )
      case 5:
        return !!(
          values.certifyAccuracy === true &&
          values.authorizeContact === true &&
          values.acknowledgeNoValidation === true
        )
      default:
        return true
    }
